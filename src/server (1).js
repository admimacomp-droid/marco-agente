  ...
  try {
    ...
    for (let turno = 0; turno < MAX_TOOL_TURNS; turno++) {
      ...
      for (const bloque of respuesta.content) {
        ...
        // El widget espera: event "tool" con campo "name"
        send("tool", { name: bloque.name });
    ...
    send("text", {
      delta:
        "Necesito que un ingeniero de IMACOMP revise tu caso para darte una respuesta precisa. ¿Me dejas tu correo o teléfono?",
    });
    send("done", { messages });
    res.end();
  } catch (e) {
    console.error("Error en /api/marco:", e.message);
    send("error", { message: "Ocurrió un problema. Intenta nuevamente." });
    res.end();
  }
});
